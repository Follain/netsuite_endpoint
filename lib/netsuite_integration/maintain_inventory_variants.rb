  ...
  class MaintainInventoryVariants < Base
    ...
    def initialize(config, payload = {})
      ...
      inventoryitem_payload['variants'].map do |line_item|
        add_sku(line_item) if line_item['changed']
      end
      raise "update/create errors #{@errors.first}" if @errors.present?
    end
    ...
    def add_sku(line_item)
      ...
      item = inventory_item_service.find_by_internal_id(ns_id) unless ns_id.nil?
      ...
      if item.errors.present? { |e| e.type != 'WARN' }
        if !item.errors.map(&:message).first.include?('Uniqu') && !item.errors.map(&:message).first.include?('different type')
